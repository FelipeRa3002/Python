...
import os , pygame
from funções import BancodeDados
class StatusAluno:
    ATIVO = "Ativo"
    PENDENTE = "Pendente"
    DESATIVO = "Desativo"
class Aluno:
    def __init__(self,nomedaturma,nome,nota,situacao):
        self.nome = nome
        self.notafinal = nota
        self.situacao = situacao
        self.status = StatusAluno.ATIVO
        self.__turma = nomedaturma
    def MudarStatus(self,novo):
        if novo == "Ativar":
            self.status = StatusAluno.ATIVO
        elif novo == "Desativar":
            self.status = StatusAluno.DESATIVO
        elif novo == "Pendente":
            self.status = StatusAluno.PENDENTE
pygame.init()
pygame.mixer.init()
...
def chamarjanelaturma(nomedaturma,professor,nomedoobjeto,urlpersonalizada,janela):
    janela.quit()
    ...
    funções = BancodeDados(urlcompleta= urlpersonalizada)
    ctk.CTkButton(**btn_params, text="Adicionar Aluno", command=lambda: Adicionar(link=urlpersonalizada)).grid(row=0, column=0, padx=50, pady=50)
    ctk.CTkButton(**btn_params, text="Mostrar os Alunos", command=lambda: Mostrar(link=urlpersonalizada)).grid(row=0, column=1, padx=50, pady=50)
    ctk.CTkButton(**btn_params, text="Apagar o Aluno", command=lambda: Apagar(link=urlpersonalizada)).grid(row=0, column=2, padx=50, pady=50)
    
    ctk.CTkButton(**btn_params, text="Explicação", command=lambda: Explicacao(link=urlpersonalizada)).grid(row=1, column=0, columnspan=3,sticky="ew", padx=50, pady=50)
    
    ctk.CTkButton(**btn_params, text="Consultar", command=lambda: Consultar(link=urlpersonalizada)).grid(row=2, column=0, padx=50, pady=50)
    ctk.CTkButton(**btn_params, text="Alterar", command=lambda: Alterar(link=urlpersonalizada)).grid(row=2, column=1, padx=50, pady=50)
    Janela.mainloop()
def CriarSubJanela():
...
def Adicionar(link):
    ...
    ctk.CTkButton(**config_interativo, hover_color= "#A7CA0A",border_width= 2,border_color="white",text="Enviar", 
                  command=lambda: BancodeDados.AdicionarNaLista(nome=CaixaDeTexto.get(), av=notaDaAV.get(), 
                                                           avs=notaDaAVS.get(),janela=JanelaSub,link=link)).grid(row=3,column=0,columnspan=2,padx=20, pady=20,sticky="ew")

def Mostrar(link):
    JanelaSub= CriarSubJanela()
    
    banco = BancodeDados(urlcompleta=link)
    texto = banco.ListarNomes()
...
def Apagar(link):
    ...
    lista_nomes = BancodeDados.ListarNomes(link)
    ...
    ctk.CTkButton(**botoes, text="Apagar", command=lambda: 
                  BancodeDados.ApagarNaLista(CaixaDeTexto.get(),janela=JanelaSub,link=link)).grid(row=2,column=0,columnspan=3)
...
def Explicacao():
    ...
    IrParaSite = ctk.CTkButton(**botoes , text="Ir para um site com a explicação", 
                            command=lambda: BancodeDados.AbrirNavegador(url="https://shre.ink/projetodepython"))
    ...
    GeraRelatorio = ctk.CTkButton(**botoes , text="Gera relatório .txt", 
                                command=lambda: BancodeDados.GerarRelatorio(tipo=".txt")) 
...
def Consultar(link):
    ...
    ctk.CTkButton(**botoes, text="Enviar", 
                  command=lambda: BancodeDados.ConsultarNaLista(nome=nome.get(),janela=JanelaSub,link=link)).grid(
...
def Alterar(link):
    ...
    texto= BancodeDados.ListarNomes(link)
    botoes = {
    "master": JanelaSub,
    "text_color": "white",
    "fg_color": "transparent",
    ...
    ctk.CTkButton(**botoes, text="Enviar", 
                command=lambda: BancodeDados.MudarNaLista(posicao=id_aluno.get(), nome=CaixaDeTexto.get(), 
                                                     av=notaDaAV.get(), avs=notaDaAVS.get(), janela=JanelaSub,link=link)).grid(row=5,column=0,columnspan=3,sticky="ew",padx=20,pady=20)
